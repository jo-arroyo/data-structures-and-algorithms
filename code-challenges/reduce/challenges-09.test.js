...
const returnNames = (arr) => {
  let nameArray = arr.reduce ((acc, val) => {
    acc.push(val.name);
    return acc;
  }, []);
  return nameArray;
...
const reversedString = (str) => {
  ...
  let reverseStr = stringy.reduce((acc, val) => {
    return val + acc;
  }, '');
  return reverseStr;
};
...
const calculateAverage = (arr) => {
  ...
  let finalNumber = (number.sum / number.count);
  return finalNumber;
};
...
const extractChildren = (arr) => {
  let hasA = arr.filter(element => element.name.match(/[a]/));
  let hasKids = hasA.filter(element => element.children);
  let children = hasKids.reduce((acc, val) => {
    acc.push(val.children);
    return acc;
  }, []);
  let answer = children.reduce((acc, val) => {
