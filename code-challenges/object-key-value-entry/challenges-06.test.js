...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(gotObj => {
    let allValues = Object.values(gotObj);
    houses.push(allValues[3]);
  });
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  ...
  Object.values(arr).forEach(person => {
    ...
    if (personInfo[0] === character){
      if (personInfo[2].length < 1){
        answer = false;
      } else {
        answer = true;
      }
...
const hasChildrenEntries = (arr, character) => {
  ...
  Object.entries(arr).forEach(entry => {
    let personInfo = Object.values(entry[1]);
    if (personInfo[0] === character){
      if (personInfo[2].length < 1){
        answer = false;
      } else {
...
const totalCharacters = (arr) => {
  ...
  arr.forEach(characterObject => {
    let person = Object.values(characterObject);
    if (!person[0] === false){
      charactersArray.push(person[0]);
    }
    if (!person[1] === false){
      charactersArray.push(person[1]);
    }
...
const houseSize = (arr) => {
  ...
  arr.forEach(obj => {
    let thing = 0;
    obj.name ? thing++ : null;
    obj.spouse ? thing++ : null;
    obj.children ? thing += obj.children.length: null;
    const newObj = { house: obj.house, members: thing}
    sizes.push(newObj);
  })
  return sizes;
};
