...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach(step => {
    let stepWord = step.split(' ');
    result.push(stepWord[0]);
  });
  return result;
};
...
const extractVowels = (str) => {
  ...
  let consonantString = '';
  let vowelString = '';
  let consonantArray = str.split(regex);
  consonantArray.forEach(segment => consonantString = consonantString + segment);
  let vowelArray = str.match(regex);
  vowelArray = vowelArray.sort();
  vowelArray.forEach(segment => vowelString = vowelString + segment);
  returnArray.push(consonantString);
  returnArray.push(vowelString);
  return returnArray;
};
